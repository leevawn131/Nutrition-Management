...
class AdminFoodService {
   ...
   */
  async listFoods({ page = 1, limit = 10, search = '', category = 'all', is_verified = 'all' }) {
  ...
  async createFood(foodData, adminId) {
    ...
    if (!foodData.name || typeof foodData.name !== 'string' || !foodData.name.trim()) {
    ...
    if (
      ...
      const error = new Error('Lượng calo/100g (calories_per_100g) phải là số không âm (>= 0).');
      error.statusCode = 400;
      throw error;
    }
    ...
    if (foodData.aliases !== undefined && foodData.aliases !== null) {
      if (!Array.isArray(foodData.aliases)) {
        const error = new Error('Trường bí danh (aliases) phải là một mảng chuỗi.');
        error.statusCode = 400;
        throw error;
      }
    ...
    const newFood = new FoodItem({
      ...
      name_en: foodData.name_en && typeof foodData.name_en === 'string' ? foodData.name_en.trim() : null,
      category: foodData.category && typeof foodData.category === 'string' ? foodData.category.trim() : null,
      calories_per_100g: Number(foodData.calories_per_100g),
      protein_per_100g:
        foodData.protein_per_100g !== undefined && foodData.protein_per_100g !== null && foodData.protein_per_100g !== ''
          ? Number(foodData.protein_per_100g)
          : null,
      carb_per_100g:
        foodData.carb_per_100g !== undefined && foodData.carb_per_100g !== null && foodData.carb_per_100g !== ''
          ? Number(foodData.carb_per_100g)
          : null,
      fat_per_100g:
        foodData.fat_per_100g !== undefined && foodData.fat_per_100g !== null && foodData.fat_per_100g !== ''
          ? Number(foodData.fat_per_100g)
          : null,
      image_url: foodData.image_url && typeof foodData.image_url === 'string' ? foodData.image_url.trim() : null,
      is_verified: foodData.is_verified !== undefined ? Boolean(foodData.is_verified) : true,
      aliases,
  ...
  async updateFood(id, updateData) {
    ...
    if (updateData.name_en !== undefined) {
      updates.name_en = updateData.name_en && typeof updateData.name_en === 'string' ? updateData.name_en.trim() : null;
    }

    if (updateData.category !== undefined) {
      updates.category = updateData.category && typeof updateData.category === 'string' ? updateData.category.trim() : null;
    }
  ...
  async deleteFood(id) {
    ...
    const [mealLogsCount, mealPlansCount, unidentifiedFoodsCount] = await Promise.all([
      mongoose.connection.collection('meal_logs').countDocuments({ food_item_id: foodObjId }),
      mongoose.connection.collection('meal_plans').countDocuments({ food_item_id: foodObjId }),
      mongoose.connection.collection('unidentified_foods').countDocuments({ resolved_food_item_id: foodObjId }),
    ]);

    if (mealLogsCount > 0 || mealPlansCount > 0 || unidentifiedFoodsCount > 0) {
      const reasons = [];
      if (mealLogsCount > 0) reasons.push(`${mealLogsCount} nhật ký ăn uống (meal_logs)`);
      if (mealPlansCount > 0) reasons.push(`${mealPlansCount} kế hoạch bữa ăn (meal_plans)`);
      if (unidentifiedFoodsCount > 0) reasons.push(`${unidentifiedFoodsCount} nhận diện món chưa phân loại`);
