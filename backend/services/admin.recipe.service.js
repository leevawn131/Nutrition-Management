...
class AdminRecipeService {
  ...
  async listRecipes({
    ...
    if (source_type && source_type !== "all") {
      if (!["system", "community"].includes(source_type)) {
        const error = new Error(
          "Tham số source_type không hợp lệ. Chỉ chấp nhận system hoặc community.",
        );
        error.statusCode = 400;
        throw error;
      }
    ...
    if (status && status !== "all") {
      if (!["pending", "approved", "rejected"].includes(status)) {
        const error = new Error(
          "Tham số status không hợp lệ. Chỉ chấp nhận pending, approved hoặc rejected.",
        );
        error.statusCode = 400;
        throw error;
      }
  ...
  async createRecipe(recipeData, adminId) {
    ...
    if (
      ...
      const error = new Error(
        "Tên công thức (title) là bắt buộc và không được để trống.",
      );
      error.statusCode = 400;
      throw error;
    }
    ...
    if (
      ...
      isNaN(Number(recipeData.servings)) ||
      Number(recipeData.servings) <= 0
    ) {
      const error = new Error(
        "Khẩu phần (servings) là bắt buộc và phải là số > 0.",
      );
      error.statusCode = 400;
      throw error;
    }
    ...
    if (
      ...
      if (!Array.isArray(recipeData.ingredients)) {
        const error = new Error(
          "Danh sách nguyên liệu (ingredients) phải là một mảng.",
        );
        error.statusCode = 400;
        throw error;
      }
      for (let i = 0; i < recipeData.ingredients.length; i++) {
        ...
        if (
          ...
          const error = new Error(
            `Nguyên liệu thứ ${i + 1} phải có tên (ingredient_name) hợp lệ.`,
          );
          error.statusCode = 400;
          throw error;
        }
        if (
          ...
          ing.quantity !== null &&
          (isNaN(Number(ing.quantity)) || Number(ing.quantity) < 0)
        ) {
          const error = new Error(
            `Định lượng nguyên liệu ${ing.ingredient_name} phải là số >= 0.`,
          );
          error.statusCode = 400;
          throw error;
        }
    ...
    if (recipeData.steps !== undefined && recipeData.steps !== null) {
      ...
      for (let i = 0; i < recipeData.steps.length; i++) {
        ...
        if (
          ...
          typeof st !== "object" ||
          !st.instruction ||
          typeof st.instruction !== "string" ||
          !st.instruction.trim()
        ) {
          const error = new Error(
            `Bước thứ ${i + 1} phải có nội dung hướng dẫn (instruction).`,
          );
          error.statusCode = 400;
          throw error;
        }
        ...
        if (isNaN(stepNum) || stepNum < 1) {
          const error = new Error(
            `Thứ tự bước thứ ${i + 1} (step_number) phải là số nguyên >= 1.`,
          );
          error.statusCode = 400;
          throw error;
        }
    ...
    if (
      recipeData.nutrition_facts &&
      typeof recipeData.nutrition_facts === "object"
    ) {
      nutrition_facts = {};
      const nfFields = [
        "energy_kcal",
      ...
      for (const f of nfFields) {
        if (
          recipeData.nutrition_facts[f] !== undefined &&
          recipeData.nutrition_facts[f] !== null &&
          recipeData.nutrition_facts[f] !== ""
        ) {
          if (
            isNaN(Number(recipeData.nutrition_facts[f])) ||
            Number(recipeData.nutrition_facts[f]) < 0
          ) {
            const error = new Error(
              `Chỉ số vi chất ${f} phải là số không âm (>= 0).`,
            );
            error.statusCode = 400;
            throw error;
          }
    ...
    const newRecipe = new Recipe({
      ...
      description:
        recipeData.description && typeof recipeData.description === "string"
          ...
          : null,
      image_url:
        recipeData.image_url && typeof recipeData.image_url === "string"
          ? recipeData.image_url.trim()
          : null,
      prep_time_minutes:
        recipeData.prep_time_minutes !== undefined &&
        recipeData.prep_time_minutes !== null &&
        recipeData.prep_time_minutes !== ""
          ? Number(recipeData.prep_time_minutes)
          : null,
      cook_time_minutes:
        recipeData.cook_time_minutes !== undefined &&
        recipeData.cook_time_minutes !== null &&
        recipeData.cook_time_minutes !== ""
          ? Number(recipeData.cook_time_minutes)
          : null,
      servings: Number(recipeData.servings),
      calories_per_serving:
        recipeData.calories_per_serving !== undefined &&
        recipeData.calories_per_serving !== null &&
        recipeData.calories_per_serving !== ""
          ? Number(recipeData.calories_per_serving)
          : null,
      protein_g:
        recipeData.protein_g !== undefined &&
        recipeData.protein_g !== null &&
        recipeData.protein_g !== ""
          ? Number(recipeData.protein_g)
          : null,
      carb_g:
        recipeData.carb_g !== undefined &&
        recipeData.carb_g !== null &&
        recipeData.carb_g !== ""
          ? Number(recipeData.carb_g)
          : null,
      fat_g:
        recipeData.fat_g !== undefined &&
        recipeData.fat_g !== null &&
        recipeData.fat_g !== ""
          ? Number(recipeData.fat_g)
          : null,
      avg_rating: 0,
      comment_count: 0,
      source_type: "system",
  ...
  async updateRecipe(id, updateData) {
    ...
    if (updateData.created_by_user_id !== undefined) {
      const error = new Error(
        "Không được phép thay đổi người tạo (created_by_user_id).",
      );
      error.statusCode = 400;
      throw error;
    }
    ...
    if (updateData.servings !== undefined) {
      if (
        isNaN(Number(updateData.servings)) ||
        Number(updateData.servings) <= 0
      ) {
    ...
    if (updateData.ingredients !== undefined) {
      ...
      for (let i = 0; i < updateData.ingredients.length; i++) {
        ...
        if (
          ...
          const error = new Error(
            `Nguyên liệu thứ ${i + 1} phải có tên hợp lệ.`,
          );
          error.statusCode = 400;
          throw error;
        }
    ...
    if (updateData.steps !== undefined) {
      ...
      for (let i = 0; i < updateData.steps.length; i++) {
        ...
        if (
          ...
          const error = new Error(
            `Bước thứ ${i + 1} phải có nội dung hướng dẫn.`,
          );
          error.statusCode = 400;
          throw error;
        }
    ...
    if (updateData.nutrition_facts !== undefined) {
      if (updateData.nutrition_facts === null) {
        ...
        for (const f of nfFields) {
          if (
            updateData.nutrition_facts[f] !== undefined &&
            updateData.nutrition_facts[f] !== null &&
            updateData.nutrition_facts[f] !== ""
          ) {
  ...
  async updateRecipeStatus(id, status) {
    ...
    if (!["approved", "rejected"].includes(status)) {
      const error = new Error(
        "Trạng thái duyệt chỉ có thể là approved hoặc rejected.",
      );
      error.statusCode = 400;
      throw error;
    }
  ...
  async deleteRecipe(id) {
    ...
    const [templateCount, mealPlanCount] = await Promise.all([
      mongoose.connection
        ...
        .countDocuments({ "items.recipe_id": recipeObjId }),
      mongoose.connection
        .collection("meal_plans")
        .countDocuments({ recipe_id: recipeObjId }),
    ]);

    if (templateCount > 0 || mealPlanCount > 0) {
      const reasons = [];
      if (templateCount > 0)
        reasons.push(`${templateCount} thực đơn mẫu (meal_plan_templates)`);
      if (mealPlanCount > 0)
        reasons.push(
          `${mealPlanCount} kế hoạch bữa ăn người dùng (meal_plans)`,
        );
