...
export const adminFoodService = {
  ...
  async getFoods(params: FoodListParams = {}): Promise<FoodListResponse> {
    const response = await apiClient.get<FoodListResponse>('/admin/foods', {
      params: {
        ...
        ...(params.is_verified && params.is_verified !== 'all' ? { is_verified: params.is_verified } : {}),
      },
    });
  ...
  async getFoodById(id: string): Promise<FoodDetailResponse> {
    const response = await apiClient.get<FoodDetailResponse>(`/admin/foods/${id}`);
    return response.data;
  },
  ...
  async createFood(payload: CreateFoodPayload): Promise<MutateFoodResponse> {
    const response = await apiClient.post<MutateFoodResponse>('/admin/foods', payload);
    return response.data;
  },
   ...
   */
  async updateFood(id: string, payload: UpdateFoodPayload): Promise<MutateFoodResponse> {
    const response = await apiClient.put<MutateFoodResponse>(`/admin/foods/${id}`, payload);
    return response.data;
  },
  ...
  async deleteFood(id: string): Promise<{ success: boolean; message: string }> {
    const response = await apiClient.delete<{ success: boolean; message: string }>(`/admin/foods/${id}`);
    return response.data;
  },
};
