...
export const adminRecipeService = {
  ...
  async getRecipeById(id: string): Promise<RecipeDetailResponse> {
    const response = await apiClient.get<RecipeDetailResponse>(`/admin/recipes/${id}`);
    return response.data;
  },
   ...
   */
  async createRecipe(payload: CreateRecipePayload): Promise<MutateRecipeResponse> {
    const response = await apiClient.post<MutateRecipeResponse>('/admin/recipes', payload);
    return response.data;
  },
   ...
   */
  async updateRecipe(id: string, payload: UpdateRecipePayload): Promise<MutateRecipeResponse> {
    const response = await apiClient.put<MutateRecipeResponse>(`/admin/recipes/${id}`, payload);
    return response.data;
  },
   ...
   */
  async updateRecipeStatus(id: string, status: 'approved' | 'rejected'): Promise<MutateRecipeResponse> {
    const response = await apiClient.put<MutateRecipeResponse>(`/admin/recipes/${id}/status`, { status });
    return response.data;
  },
  ...
  async deleteRecipe(id: string): Promise<{ success: boolean; message: string }> {
    const response = await apiClient.delete<{ success: boolean; message: string }>(`/admin/recipes/${id}`);
    return response.data;
  },
};
